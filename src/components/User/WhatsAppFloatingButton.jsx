...
const WhatsAppFloatingButton = React.memo(({ 
  ...
  // Memoize the final WhatsApp link to prevent recalculation
  const finalWhatsAppLink = useMemo(() => {
    if (phoneNumber) {
      const encodedMessage = encodeURIComponent(message);
      return `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    }
    return whatsappLink;
  }, [phoneNumber, message, whatsappLink]);
  ...
  // Single callback for WhatsApp click
  const handleWhatsAppClick = useCallback(() => {
    // Optional analytics tracking (only if gtag exists)
  ...
  return (
    <div className={`${styles.whatsappContainer} ${positionClass}`}>
      ...
      >
        ...
        {/* Online indicator dot - Pure CSS animation */}
        <div className={styles.onlineIndicator}></div>
      </button>

      {/* Tooltip - Only render if showTooltip is true */}
      {showTooltip && (
